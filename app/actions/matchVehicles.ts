...
function calculateMatchingScore(
  ...
  if (user.dailyUsage === "shortCityTrips")
    attributeScores.range = normalize(vehicle.attributes.range, 100, 250);
  else if (user.dailyUsage === "commuting")
    attributeScores.range = normalize(vehicle.attributes.range, 250, 400);
  else if (user.dailyUsage === "mixedUse")
    attributeScores.range = normalize(vehicle.attributes.range, 300, 500);
  else attributeScores.range = normalize(vehicle.attributes.range, 400, 800);

  if (user.passengers === "alone")
    attributeScores.seats = vehicle.attributes.seats >= 2 ? 1 : 0;
  else if (user.passengers === "withOnePerson")
    attributeScores.seats = vehicle.attributes.seats >= 2 ? 1 : 0;
  else if (user.passengers === "smallFamily")
    attributeScores.seats = normalize(vehicle.attributes.seats, 4, 5);
  else attributeScores.seats = normalize(vehicle.attributes.seats, 5, 7);

  if (user.luggageSpace === "small")
    attributeScores.trunkVolume = normalize(
  ...
  const bodyTypePreferenceMatch =
    (user.passengers === "smallFamily" &&
      vehicle.attributes.bodyType === "suv") ||
    (user.passengers === "largeFamily" &&
      ["suv", "stationWagon"].includes(vehicle.attributes.bodyType)) ||
    (user.passengers === "alone" &&
      vehicle.attributes.bodyType === "hatchback");
  attributeScores.bodyType = bodyTypePreferenceMatch ? 1 : 0.5;

  if (
    user.dailyUsage === "longTrips" ||
    user.longDistanceFrequency === "regularly"
  )
    attributeScores.acceleration = normalize(
  ...
  if (user.chargingOption === "privateGarage")
    attributeScores.chargingTime = normalize(
